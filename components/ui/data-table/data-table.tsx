...
export default function DataTable<T>({
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 px-4 md:px-6">
        <div>
              ...
              <div key={String(item[keyField])}>
                {renderMobileCard ? (
                  ...
                  <div className="border border-gray-200 rounded-lg p-3 bg-white">
                    ...
                    {columns.length > 0 && columns[0].cell && (
                      <div className="mb-2 pb-2 border-b border-gray-100">
                        {columns[0].cell(item)}
                      </div>
                    )}
                    {/* Remaining non-action columns as compact rows */}
                    <div className="space-y-1.5">
                      {columns.slice(1).filter(c => c.header !== "Actions").map((column, index) => (
                        <div key={index} className="flex items-center justify-between gap-2 text-sm">
                          <span className="text-xs text-gray-500 shrink-0">{column.header}</span>
                    ...
                    {/* Actions */}
                    {(renderRowActions || columns.some(c => c.header === "Actions")) && (
                      <div className="flex justify-end gap-2 pt-2 mt-2 border-t border-gray-100">
                        {renderRowActions
                          ? renderRowActions(item)
                          : columns.find(c => c.header === "Actions")?.cell?.(item)}
                      </div>
                    )}
                  </div>
                )}
